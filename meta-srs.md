...
- Receive Messenger messages and postback events over webhook
- Send auto-reply to incoming messages and postback events
...
| `pages_messaging` | Send replies on behalf of the Page (covers both message replies and postback responses) |
| `pages_manage_metadata` | Subscribe the Page to our webhook |

> **Note:** OAuth scopes and webhook subscription fields (Section 3.3) are independent permission systems. Adding a new subscription field (e.g. `messaging_postbacks`) does **not** require a new OAuth scope as long as `pages_messaging` is already granted.
...
The same subscription fields must also be passed when calling `POST /{page-id}/subscribed_apps?subscribed_fields=messages,messaging_postbacks` during the connect flow (Section 4.2).
...
- Each Page is subscribed to the app's webhook with `subscribed_fields=messages,messaging_postbacks`.
...
- Two event types must be handled:
  - **`messages`** — for each incoming text message, pick a reply and send it via the Messenger Send API.
  - **`messaging_postbacks`** — for each postback event (e.g. user taps a button, Get Started, persistent menu item), pick a reply based on the postback `payload` and send it via the Messenger Send API.
- The endpoint must return `200` within Meta's retry window regardless of event type.
...
- **Privacy**
  - Logs do not contain raw message content, postback payloads, or access tokens.
...
2. Sending a text message to a connected Page produces an auto-reply within a few seconds.
3. Triggering a postback on a connected Page (e.g. tapping a button or the Get Started action) produces an auto-reply within a few seconds.
4. `/privacy` and `/terms` are reachable without authentication and are linked in Meta App settings.
5. `GET /webhook/facebook` with the correct verify token returns the challenge string; an incorrect token returns 403.
6. `POST /webhook/facebook` with a valid signature is accepted; an invalid signature is rejected.
